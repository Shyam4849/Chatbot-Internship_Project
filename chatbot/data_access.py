...
from .config import MASTER_EXCEL_PATH, logger
...
def _excel_file_exists() -> bool:
    ...
    if not exists:
        logger.error(f"Master Excel file not found at {MASTER_EXCEL_PATH}")
    return exists
...
def _sheet_exists(sheet_name: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Failed to open Excel file for sheet validation: {e}")
        return False
